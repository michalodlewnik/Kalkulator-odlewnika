...
st.markdown("""
    ...
    h1 { font-size: 50px !important; color: #00BFFF !important; text-align: center; padding-bottom: 15px !important; }
    h3 { font-size: 30px !important; color: white !important; padding-top: 10px; }
...
# 10. DODATKOWY SUWAK UZYSKU (DLA CIEBIE)
st.subheader("⚙️ Parametry Procesu")
...
if st.button("🔄 RESETUJ DO USTAWIEŃ DOMYŚLNYCH"):
    st.session_state.topseed_val = domyslny_topseed
    st.session_state.kubek_val = domyslny_kubek
    st.rerun()
...
st.markdown(f'<div class="result-box"><div style="font-size: 25px;">ILOŚĆ ZAPRAWY (Uzysk {uzysk_custom}%)</div><div class="result-val">{ilosc_zaprawy:.1f} kg</div></div>', unsafe_allow_html=True)
