...
# --- 1. LOGIKA PAMIĘCI (Zabezpieczenie przed zerami i błędami) ---
# Inicjalizacja musi być wykonana przed wywołaniem pól
if 'masa_last' not in st.session_state:
    st.session_state.masa_last = 1100.0

# --- 2. LAYOUT I STYLE (Twoje idealne podpunkty) ---
st.markdown("""
    <style>
...
# Resetowanie wartości przy zmianie masy lub przy pierwszym uruchomieniu
if (masa != st.session_state.masa_last) or ('topseed_input' not in st.session_state):
    st.session_state.topseed_input = domyslny_topseed
    st.session_state.kubek_input = domyslny_kubek
    st.session_state.masa_last = masa
